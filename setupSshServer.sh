...
ssh -t $remoteUser@$remoteIpAddress "echo PasswordAuthentication no | sudo tee -a /etc/ssh/sshd_config && echo Port $remotePort | sudo tee -a /etc/ssh/sshd_config && echo ChallengeResponseAuthentication no | sudo tee -a /etc/ssh/sshd_config && sudo systemctl restart sshd"
